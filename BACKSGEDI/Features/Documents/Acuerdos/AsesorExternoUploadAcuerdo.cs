...
public record AsesorExternoUploadAcuerdoRequest
{
...
public class AsesorExternoUploadAcuerdoValidator : Validator<AsesorExternoUploadAcuerdoRequest>
{
    public AsesorExternoUploadAcuerdoValidator(Microsoft.Extensions.Options.IOptions<BACKSGEDI.Configuration.StorageOptions> options)
    {
        RuleFor(x => x.File)
            .NotNull()
...
public class AsesorExternoUploadAcuerdo : Endpoint<AsesorExternoUploadAcuerdoRequest>
{
    private readonly ApplicationDbContext _db;
    private readonly IStorageService _storageService;
    private readonly BACKSGEDI.Domain.Interfaces.IFechasLimiteService _fechasLimiteService;

    public AsesorExternoUploadAcuerdo(ApplicationDbContext db, IStorageService storageService, BACKSGEDI.Domain.Interfaces.IFechasLimiteService fechasLimiteService)
    {
        _db = db;
    ...
    {
        Post("/api/asesor-externo/documentos/acuerdos");
        Roles(SystemRoles.AsesorExterno);
        AllowFileUploads();
    }

    public override async Task HandleAsync(AsesorExternoUploadAcuerdoRequest req, CancellationToken ct)
    {
        var userId = User.GetUserId();

        var asesor = await _db.AsesoresExternos.FirstOrDefaultAsync(a => a.UsuarioId == userId, ct);
        if (asesor == null)
        {
            await Result.Failure(Error.NotFound("AsesorExterno.NotFound", "No se encontró el perfil de asesor externo."))
                .ToResult().ExecuteAsync(HttpContext);
            return;
        }

        var alumno = await _db.Alumnos.FirstOrDefaultAsync(a => a.Id == req.AlumnoId && a.AsesorExternoId == asesor.Id, ct);
        if (alumno == null)
        {
            await Result.Failure(Error.Forbidden("AsesorExterno.NotAssigned", "Este alumno no está asignado a tu asesoría."))
                .ToResult().ExecuteAsync(HttpContext);
            return;
        }

        if (!DocumentoPermissions.CanUpload(req.TipoAcuerdo, [SystemRoles.AsesorExterno]))
        {
